...
public class GlobalExceptionHandler {
   ...
   *
   * @param exception The EmailAlreadyExistsException thrown when an email already exists.
   * @param request   The HttpServletRequest object containing request details.
   * @return A ResponseEntity containing an ApiError object with error details.
   */
  ...
  public ResponseEntity<ApiError> handleEmailAlreadyExistsException(
    ...
    return new ResponseEntity<>(
        new ApiError(LocalDateTime.now(), HttpStatus.CONFLICT.value(), "Email already exists",
            exception.getMessage(), request.getRequestURI()), HttpStatus.CONFLICT);
  }
